...
import com.yowyob.easyrental.modules.auth.application.UserUseCaseImpl;
...
public class UserController {
    ...
    public Mono<ResponseEntity<UserEntity>> updateProfile(@RequestBody @Valid UserProfileUpdateDTO dto) {
        return authUseCaseImpl.getCurrentUser()
                .flatMap(user -> userUseCaseImpl.updateProfile(user.getId(), dto))
    ...
    public Mono<ResponseEntity<String>> updatePassword(@RequestBody @Valid PasswordUpdateDTO dto) {
        return authUseCaseImpl.getCurrentUser()
                .flatMap(user -> userUseCaseImpl.updatePassword(user.getId(), dto))
    ...
    public Flux<PermissionEntity> getMyPermissions() {
        return authUseCaseImpl.getCurrentUser()
                .flatMapMany(user -> userUseCaseImpl.getUserPermissions(user.getId()));
    }
}
